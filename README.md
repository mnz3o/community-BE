...
데이터가 증가할수록 정렬 과정에서 발생하는 비용을 고려할 필요가 있었습니다.

또한 게시글 조회 시 Soft Delete된 데이터를 제외하기 위해
`deleted_at IS NULL` 조건이 반복적으로 사용되고 있었습니다.

#### 개선

실제 게시글 목록 API에서 사용하는 조회 조건과 정렬 순서를 기준으로
복합 인덱스를 설계했습니다.

조회수순 목록은 다음과 같은 조건을 사용합니다.

`WHERE deleted_at IS NULL`

`ORDER BY view_count DESC, post_id DESC`

따라서 조회 조건과 정렬 순서를 고려하여 다음 복합 인덱스를 적용했습니다.

`(deleted_at, view_count DESC, post_id DESC)`

좋아요순 역시 동일한 방식으로 다음 인덱스를 적용했습니다.

`(deleted_at, like_count DESC, post_id DESC)`

`post_id`를 함께 포함하여 조회수나 좋아요 수가 동일한 경우에도
커서 기반 페이지네이션의 보조 정렬 기준과 인덱스의 컬럼 순서가 일치하도록 구성했습니다.

#### 검증

인덱스를 추가하는 것에서 끝내지 않고
`EXPLAIN`과 `EXPLAIN ANALYZE`를 통해 실제 쿼리의 실행 계획을 확인했습니다.

특히 `deleted_at IS NULL` 조건과
`view_count DESC, post_id DESC` 정렬을 사용하는 조회 쿼리를 기준으로
설계한 복합 인덱스가 실제 실행 계획에서 사용되는지 확인했습니다.

좋아요순 조회 역시 동일한 방식으로 실행 계획을 확인하여
조회 패턴에 맞게 구성한 인덱스가 사용되는지 검증했습니다.

#### 결과

게시글의 조회 조건과 정렬 조건을 기준으로 복합 인덱스를 구성하고,
실행 계획을 통해 실제 쿼리에서 인덱스가 사용되는지 검증했습니다.

또한 커서 기반 페이지네이션에서 사용하는
`정렬 기준값 + post_id` 구조와 인덱스의 컬럼 순서를 맞춰
페이지네이션 쿼리와 인덱스 설계를 일관되게 구성했습니다.
...
게시글 목록 조회 시 각 게시글 작성자의 프로필 이미지를 가져오기 위해
게시글마다 `StoredFile`을 추가로 조회하고 있었습니다.

```
storedFileRepository.findByUserAndIsActiveTrue(user);
```

이 구조에서는 게시글 목록을 한 번 조회한 이후
게시글 수만큼 프로필 이미지 조회 쿼리가 추가로 발생했습니다.

예를 들어 게시글 20개를 조회하면 게시글 목록 조회 1회와
프로필 이미지 조회 20회로 총 21회의 쿼리가 실행될 수 있었습니다.

#### 원인

게시글 목록을 먼저 조회한 뒤 Service 계층에서 각 게시글을 순회하면서
작성자의 프로필 이미지를 개별적으로 조회하고 있었기 때문입니다.

게시글 수가 증가할수록 추가 조회 역시 증가하는 N+1 구조였습니다.

#### 해결

QueryDSL을 이용하여 게시글 목록을 조회할 때
작성자의 활성화된 `StoredFile`을 함께 조회하도록 변경했습니다.
...
또한 DTO Projection을 적용하여 게시글 정보와 작성자의 프로필 이미지 URL을
한 번의 조회 결과에서 바로 생성하도록 구성했습니다.
...
기존의 `1 + N` 형태로 발생하던 추가 조회를 제거하여
게시글과 작성자의 프로필 정보를 함께 조회할 수 있도록 개선했습니다.

게시글 수가 증가함에 따라 프로필 이미지 조회 쿼리도 함께 증가하던 문제를 해결하고,
불필요한 데이터베이스 접근을 줄였습니다.

---

### 2. 프로필 이미지 수정 시 500 Internal Server Error

#### 문제

사용자가 기존 프로필 이미지를 새로운 이미지로 변경할 때
`500 Internal Server Error`가 발생했습니다.

#### 원인

프로필 이미지는 사용자와 1:1로 관리하고 있었지만,
이미지를 변경할 때마다 새로운 `StoredFile` Entity를 생성하여 저장하고 있었습니다.

즉, 기존 프로필 이미지 정보가 존재하는 상황에서도 새로운 레코드를 `INSERT`하는 방식으로 처리하여
프로필 이미지 데이터 관리 방식과 실제 관계가 맞지 않는 문제가 있었습니다.

#### 해결

프로필 이미지 변경 시 기존 `StoredFile`의 존재 여부를 먼저 확인하도록 변경했습니다.
...
기존 프로필 이미지 정보가 존재하면 새로운 Entity를 생성하지 않고
`updateFileUrl()`을 통해 기존 레코드의 URL만 변경하도록 수정했습니다.

기존 레코드가 존재하지 않는 경우에만 새로운 `StoredFile`을 생성하도록 처리했습니다.

#### 결과

프로필 이미지 변경 시 불필요한 `INSERT`가 발생하지 않고
기존 프로필 이미지 레코드가 `UPDATE`되도록 수정했습니다.

이를 통해 프로필 이미지 수정 과정에서 발생하던 `500 Internal Server Error`를 해결하고,
사용자와 프로필 이미지의 관계에 맞게 파일 데이터를 관리하도록 개선했습니다.
